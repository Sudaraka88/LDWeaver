...
#'
#' @param lr_tophits_path Path to Tophits/lr_tophits.tsv file.If LR link tophits should be visualised, the two lr_files (below) must also be provided here.
#' @param lr_fasta_path Path to LR_GWESExplorer/snps.aln file.
#' @param lr_pos_path Path to LR_GWESExplorer/snps.loci file.
#'
#' @param sr_tophits_path Path to Tophits/sr_tophits.tsv file. If SR link tophits should be visualised, the two sr_files (below) must also be provided here.
#' @param sr_fasta_path Path to SR_GWESExplorer/snps.aln file.
#' @param sr_pos_path Path to SR_GWESExplorer/snps.loci file.
#'
...
view_tree = function(tree_path, perform_midpoint_rooting = T, metadata_df = NULL,
                     lr_tophits_path = NULL, lr_fasta_path = NULL, lr_pos_path = NULL,
                     sr_tophits_path = NULL, sr_fasta_path = NULL, sr_pos_path = NULL,
  ...
  if(!is.null(lr_tophits_path)) {
    lr_tophits_path = normalizePath(lr_tophits_path)
  ...
  if(!is.null(sr_tophits_path)) {
    sr_tophits_path = normalizePath(sr_tophits_path)
  ...
  if(vis_lrsr){ # both must be read
    sr_tophits = cbind(LDWeaver::read_TopHits(sr_tophits_path), link = "sr")
    ...
    top_hits = cbind(LDWeaver::read_TopHits(sr_tophits_path), link = "sr")
  }
