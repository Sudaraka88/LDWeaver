...
#'
#' @export
make_gwes_plots = function(lr_links, sr_links, plt_folder = NULL){
  ...
  if(!lr_links_ready){
    warning("lr_links must either be (1) a data.frame with lr_links or (2) the path to the saved tsv file from perform_MI_computation()")
    return(-1)
  }
  ...
  if(!sr_links_ready){
    warning("sr_links must either be (1) a data.frame with sr_links or (2) the path to the saved tsv file from perform_MI_computation()")
    return(-1)
  }
